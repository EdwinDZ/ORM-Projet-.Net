        ...
        public Boolean Execute(String req, List<DboParameter> Param)
        {
            if (this.Connection() == true)
            {
                ...
                catch (NpgsqlException ex)
                {
                    ...
                    {
                        case 203:
                            throw new ORMExceptionsQueryPostGreSql("La requête a renvoyé plusieurs lignes mais les variables spécifiées ne sont pas des tableaux. La commande SELECT n'était pas unique.", ex);
                            return false;
                        case 208:
                            throw new ORMExceptionsQueryPostGreSql("La requête était vide. (Cela ne peut normalement pas se produire dans un programme Embedded SQL, ce qui peut indiquer une erreur interne.)", ex);
                            return false;
                        default:
                            return false;
                    }
